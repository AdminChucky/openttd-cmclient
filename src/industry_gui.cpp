...
class BuildIndustryWindow : public Window {
	...
	void SetupArrays()
	{
		...
		if (this->selected_type == IT_INVALID && !this->list.empty()) {
	...
	void SetButtons()
	{
		this->SetWidgetDisabledState(WID_DPI_FUND_WIDGET,  !this->cm_funding_enabled || (this->selected_type != IT_INVALID && !this->enabled));
...
public:
		...
		switch (widget) {
			...
			case WID_DPI_MATRIX_WIDGET: {
				...
				if (it != this->list.end()) { // Is it within the boundaries of available data?
					_cm_funding_type = this->selected_type = *it;
					this->UpdateAvailability();
	...
	void OnPlaceObject([[maybe_unused]] Point pt, TileIndex tile) override
	{
		...
		uint32_t layout_index = _cm_funding_layout;
		...
		if (_game_mode == GM_EDITOR) {
			...
			_cm_funding_layout = InteractiveRandomRange((uint32_t)indsp->layouts.size());
		} else {
