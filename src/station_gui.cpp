	...
	void OnDropdownSelect(WidgetID widget, int index, int) override
	{
		...
		if (widget == WID_STL_CARGODROPDOWN) {
			...
			if (oldstate.cargoes != this->filter.cargoes || oldstate.include_no_rating != this->filter.include_no_rating) {
				...
				if (citymania::_fn_mod) ReplaceDropDownList(this, this->BuildCargoDropDownList(this->filter_expanded));
			}

			/* Always close the list if ctrl is not pressed. */
			if (!citymania::_fn_mod) this->CloseChildWindows(WC_DROPDOWN_MENU);
		}
	}
...
struct StationViewWindow : public Window {
	...
	int cm_ratings_list_y = 0; ///< Y coordinate of first line in station ratings panel.
