...
struct BuildRoadToolbarWindow : Window {
	...
	void OnClick([[maybe_unused]] Point pt, WidgetID widget, [[maybe_unused]] int click_count) override
	{
		...
		switch (widget) {
			case WID_ROT_ROAD_X:
				HandlePlacePushButton(this, WID_ROT_ROAD_X, GetRoadTypeInfo(this->roadtype)->cursor.road_nwse, HT_RECT, DDSP_PLACE_ROAD_X_DIR);
				this->last_started_action = widget;
				break;

			case WID_ROT_ROAD_Y:
				HandlePlacePushButton(this, WID_ROT_ROAD_Y, GetRoadTypeInfo(this->roadtype)->cursor.road_swne, HT_RECT, DDSP_PLACE_ROAD_Y_DIR);
				this->last_started_action = widget;
				break;

			case WID_ROT_AUTOROAD:
				HandlePlacePushButton(this, WID_ROT_AUTOROAD, GetRoadTypeInfo(this->roadtype)->cursor.autoroad, HT_RECT, DDSP_PLACE_AUTOROAD);
				this->last_started_action = widget;
				break;
			...
			case WID_ROT_DEPOT:
				if (HandlePlacePushButton(this, WID_ROT_DEPOT, GetRoadTypeInfo(this->roadtype)->cursor.depot, HT_RECT, CM_DDSP_BUILD_ROAD_DEPOT)) {
			...
			case WID_ROT_BUILD_WAYPOINT:
				this->last_started_action = widget;
				if (HandlePlacePushButton(this, WID_ROT_BUILD_WAYPOINT, SPR_CURSOR_WAYPOINT, HT_RECT, DDSP_BUILD_ROAD_WAYPOINT)) {
			...
			case WID_ROT_BUILD_TUNNEL:
				HandlePlacePushButton(this, WID_ROT_BUILD_TUNNEL, GetRoadTypeInfo(this->roadtype)->cursor.tunnel, HT_SPECIAL, CM_DDSP_BUILD_ROAD_TUNNEL);
				this->last_started_action = widget;
				break;
			...
			case WID_ROT_CONVERT_ROAD:
				HandlePlacePushButton(this, WID_ROT_CONVERT_ROAD, GetRoadTypeInfo(this->roadtype)->cursor.convert_road, HT_RECT | HT_DIAGONAL, DDSP_CONVERT_ROAD);
				this->last_started_action = widget;
				break;
				...
				case DDSP_REMOVE_BUSSTOP:
					if (this->IsWidgetLowered(WID_ROT_BUS_STATION) && GetIfClassHasNewStopsByType(RoadStopClass::Get(_roadstop_gui.sel_class), RoadStopType::Bus, _cur_roadtype)) {
						if (_remove_button_clicked) {
							...
							PlaceRoadStop(start_tile, end_tile, RoadStopType::Bus, citymania::_fn_mod, _cur_roadtype, str);
						}
					}
					break;
				...
				case DDSP_REMOVE_TRUCKSTOP:
					if (this->IsWidgetLowered(WID_ROT_TRUCK_STATION) && GetIfClassHasNewStopsByType(RoadStopClass::Get(_roadstop_gui.sel_class), RoadStopType::Truck, _cur_roadtype)) {
						if (_remove_button_clicked) {
							...
							PlaceRoadStop(start_tile, end_tile, RoadStopType::Truck, citymania::_fn_mod, _cur_roadtype, str);
						}
					}
					break;
...
public:
	BuildRoadStationWindow(WindowDesc &desc, Window *parent, RoadStopType rs) : PickerWindow(desc, parent, TRANSPORT_ROAD, GetRoadStopPickerCallbacks(rs))
	{
		...
		if (RoadTypeIsTram(_cur_roadtype) && _roadstop_gui.orientation == citymania::STATIONDIR_AUTO) {
			_roadstop_gui.orientation = citymania::STATIONDIR_AUTO_XY;
		}
	...
	{
		if (_roadstop_gui.orientation < citymania::STATIONDIR_AUTO) {
			this->RaiseWidget(WID_BROS_STATION_NE + _roadstop_gui.orientation);
			if (_roadstop_gui.orientation < DIAGDIR_END) {
				_roadstop_gui.orientation = ChangeDiagDir(_roadstop_gui.orientation, DIAGDIRDIFF_90RIGHT);
			} else if (_roadstop_gui.orientation == citymania::STATIONDIR_X) {
				_roadstop_gui.orientation = citymania::STATIONDIR_Y;
			} else if (_roadstop_gui.orientation == citymania::STATIONDIR_Y) {
				_roadstop_gui.orientation = citymania::STATIONDIR_X;
			}
			this->LowerWidget(WID_BROS_STATION_NE + _roadstop_gui.orientation);
		} else {
			citymania::RotateAutodetection();
	...
	void OnPaint() override
	{
		...
		r.top = DrawStationCoverageAreaText(r, sct, rad, true) + WidgetDimensions::scaled.vsep_normal;
		r.top = DrawStationAuthorityText(r.left, r.right, r.top);
	...
	static inline HotkeyList road_hotkeys{"buildroadstop", {
		...
		Hotkey(CM_WKC_MOUSE_MIDDLE, "cm_rotate", CM_PCWHK_ROTATE),
	}};
...
void InitializeRoadGui()
{
	...
	_roadstop_gui.orientation = (DiagDirection)(DIAGDIR_END + 3);
